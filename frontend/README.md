...
- 对话 SSE：**`POST /travel/chat/{conversationId}`** + `Content-Type: application/json`、体 `{"query":"…"}`、`Accept: text/event-stream`；用 **`fetch` + ReadableStream** 解析（`EventSource` 不便携带 `Authorization: Bearer`）。
