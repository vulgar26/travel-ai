...
| [docs/eval/P1_HARNESS_GAP.md](docs/eval/P1_HARNESS_GAP.md) | **P1-0 harness**：评测 `meta` 已具备字段 vs 缺口清单与分步实现建议（`context_truncated` 原因含检索候选上限与改写截断等；`config_snapshot_hash` / `context_token_estimate` 近似等）；回放断点 **schema** 见 [`docs/eval/EVAL_REPLAY_CHECKPOINT.md`](docs/eval/EVAL_REPLAY_CHECKPOINT.md)（Flyway V3） |
