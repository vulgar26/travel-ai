...
- [x] P1-0 harness：provider usage 额外可观测与超时收口：`provider_usage_available` / `provider_usage_failure_reason` + `app.eval.llm-real-timeout-ms`  
- [x] P1-0 harness：real usage 探针默认按 `eval_tags` 前缀门禁（默认 `cost/*`），`provider_usage_failure_reason` 含 `tag_gate_no_tags` / `tag_gate_no_match`  
