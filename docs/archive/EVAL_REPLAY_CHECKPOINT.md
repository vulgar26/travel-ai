...
**状态**：Flyway **`V3__eval_replay_checkpoint.sql`** 已落表；**`EvalCheckpointRepository`** + **`EvalChatService#maybePersistEvalCheckpoint`** 在评测响应组装末尾 **UPSERT**（须请求体 **`conversation_id`** 非空且 plan 已解析；失败只打日志）。**读路径 + stub 续跑**已实现：校验 **effective plan** 的 SHA-256 与库中一致；不一致则 **`EVAL_CHECKPOINT_PLAN_MISMATCH`** 且**不写库**（避免冲掉旧断点）；若断点显示流水线已跑完则 **`EVAL_CHECKPOINT_RESUMED_EXHAUSTED`**；否则 **`meta.eval_checkpoint_outcome=resumed`** 并从 `last_completed_stage` 之后由 **`EvalLinearAgentPipeline#runStubStagesResumingAfterIndex`** 接着跑。**同一 `query`（`query_sha256`）** 下可复用 **`evidence_*` 快照**（含 **空列表**，对应零命中）；续跑且已复用检索快照时 **不再重复执行**与首轮相同的 **检索后门控**（业务空命中 / 业务低置信启发 / `eval_rag_scenario` stub），否则 rag stub 会卡死第二轮。  
...
| **`detail`** | JSONB：当前写入 **`request_id`**、**`behavior`**、可选 **`error_code`**（**不含** query 原文）、**`resume_eligible`**（是否仍可能从下一阶段续跑：由 **本 plan 投影流水线的最后一阶段**与 **`last_completed_stage`** 比较得出），以及用于复用的 **`query_sha256`**、**`evidence_retrieval_hits` / `evidence_sources`**（检索快照，**可为空数组**）与 **`tool_*` / `eval_tool_scenario`**（工具快照）。 |
...
3. **回放契约**：请求体带 `conversation_id` 时，服务端校验 **effective plan 哈希与库中一致**；若断点显示已结束则返回 `EVAL_CHECKPOINT_RESUMED_EXHAUSTED`；若哈希不一致返回 `EVAL_CHECKPOINT_PLAN_MISMATCH`（且**不覆盖**断点行）。  
