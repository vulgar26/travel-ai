...
| **回放 / 断点恢复**（`plan_raw_hash`、按 `conversationId` 恢复 stage） | **已收口（本仓 P1-0）**：Flyway V3；**`EvalCheckpointRepository`**；写库 **`UPSERT`**；读库 **plan 指纹校验**、`EVAL_CHECKPOINT_PLAN_MISMATCH` / **`EVAL_CHECKPOINT_RESUMED_EXHAUSTED`**、**`meta.eval_checkpoint_outcome`**；**stub 续跑**（`EvalLinearAgentPipeline#runStubStagesResumingAfterIndex`）；**同 query** 下 **证据 / 工具快照复用**（含 **零命中** 时的空列表键，避免复用失败）；**`resume_eligible`** 与投影流水线终点对齐；plan 不一致响应 **不写库**；续跑且已复用检索快照时 **跳过重复的检索后门控**（避免 rag stub 卡死第二轮）。说明见 [**`EVAL_REPLAY_CHECKPOINT.md`**](EVAL_REPLAY_CHECKPOINT.md)；集成测见 **`TravelAiApplicationIntegrationTest#evalCheckpointResumeAfterRagEmptyStubWithSameConversation`**。 | 后续（非阻塞）：Tokenizer 真预算、Supervisor-Worker 等仍属 SSOT 扩展项 |
...
11. **回放断点读路径 + 真续跑**：plan 校验、stub 从 `last_completed_stage` 之后接着跑、空证据快照、门控去重 —— **已完成**（集成测 `TravelAiApplicationIntegrationTest#evalCheckpointResumeAfterRagEmptyStubWithSameConversation` 等）。
