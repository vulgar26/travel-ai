...
| 用户反馈（P1-3） | Postgres **`user_feedback`**（Flyway **V4**）；**`POST /travel/feedback`**（`thumb` / `rating` / `comment` + 可选 `eval_case_id`、`eval_tags`、`request_id`、`conversation_id`）；**`GET /travel/feedback`** 分页列表；**JWT** 隔离；`app.feedback.max-comment-chars` / `max-page-size` |
...
| [`eval/EVAL_REPLAY_CHECKPOINT.md`](eval/EVAL_REPLAY_CHECKPOINT.md) | 评测回放 / 断点：表结构与设计（V3） |
