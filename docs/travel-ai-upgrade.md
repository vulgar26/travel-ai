...
- **反馈闭环（P1-3 基线）**：Postgres **`user_feedback`**（Flyway **V4**）；**`POST /travel/feedback`**、**`GET /travel/feedback`**（JWT 用户隔离）；**`thumb`**（`up`/`down`）、**`rating`**（1–5）、可选 **`comment`** 与 **`eval_case_id` / `eval_tags` / `request_id`**（与评测 case、标签、Agent `request_id` 对齐）；`app.feedback.*`（见 `application.yml`）。
- **主线聊天 SSE（`UPGRADE_PLAN` P5-2）**：**推荐** `POST /travel/chat/{conversationId}` + JSON `{"query":"…"}`（`Content-Type: application/json`、`Accept: text/event-stream`）；**GET** `/travel/chat/{conversationId}?query=…` **仍兼容**，响应头 **`Deprecation: true`**（并带 `Link` 提示）。**`app.conversation.max-query-chars`**（默认 8192）对 GET/POST **共用**超长校验（超出 **400** JSON）。实现：`TravelController`、`TravelChatRequest`。
- **评测口 token / usage 可观测（P1-0 小步 + 成本门禁）**：评测响应 `meta` 含字符级 **`context_*`**、默认 **`token_source=estimate`**；可选 **`llm_mode=real`** 且服务端 **`app.eval.llm-real-enabled=true`** 时触发一次真实 LLM 调用以写入 **`prompt_tokens` / `completion_tokens` / `total_tokens`**（`token_source=provider`）。默认 **`app.eval.llm-real-require-tag-match=true`**：须请求体 **`eval_tags`** 中至少一条以 **`app.eval.llm-real-required-tag-prefixes`**（默认仅 **`cost/`**）为前缀，否则跳过探针并写 **`meta.provider_usage_failure_reason`** 为 **`tag_gate_no_tags`** / **`tag_gate_no_match`**；另有 `timeout` / `no_usage` / `no_client` / `error` 等。配置：`AppEvalProperties`；策略：`EvalLlmRealTagPolicy`；主线 SSE 日志 **`[usage]`**（best effort，见 `TravelAgent`）。细节见 **`README.md`**（`app.eval.*`）、**[`docs/eval/P1_HARNESS_GAP.md`](eval/P1_HARNESS_GAP.md)**、**[`docs/eval/LLM_REAL_USAGE_RUNBOOK.md`](eval/LLM_REAL_USAGE_RUNBOOK.md)**（何时开 real、抽样与合规）。
...
- **对抗与安全样例**：工程侧有门控与错误码；**已定稿**：`docs/eval.md` §「评测口：对抗与安全 + RAG/tool 确定性用例」给出与 **`EvalChatSafetyGate` / `EvalQuerySafetyPolicy` / `EvalBehaviorPolicy`** 对齐的示例 `query`、预期 `behavior`/`error_code` 及**建议数据集 `tags`**（便于导入 Vagent 与分桶）。**仍待推进**：把行表批量导入为 ≥20 条正式 case、对**公网 target** 的全量跑（与 **默认 CI** 边界见 [**`docs/eval/CI_AND_REMOTE_EVAL.md`**](eval/CI_AND_REMOTE_EVAL.md)）。
...
- **P1-0 harness 扩展项**：如 **Tokenizer 级真预算**、Supervisor-Worker 等：**未**按 SSOT 满配。**评测断点恢复（本仓已收口）**：**Flyway V3** + **`eval_conversation_checkpoint`** 的 **写库 + 读库**（plan 指纹、`EVAL_CHECKPOINT_*`）、**stub 续跑**、**同 query 证据/工具快照复用**（含零命中空列表）、plan 不一致时 **不写库**；见 [**`docs/eval/EVAL_REPLAY_CHECKPOINT.md`**](eval/EVAL_REPLAY_CHECKPOINT.md)。**已收口小步**：`meta.context_truncated` + `context_truncation_reasons[]`（含 snippet / 工具输出 / **检索候选上限** / **非 EVAL 改写单行截断** 等可归因码）、`meta.config_snapshot_hash`（含 `alg/scope`）、**`meta.config_snapshot_id`**（与 hash 一一对应的稳定引用串）、可选 **`meta.config_snapshot`**（**`app.eval.config-snapshot-meta-enabled`**，与 hash 同源白名单键值）、字符级近似的 `meta.context_token_estimate/context_char_count`、默认 **`token_source=estimate`**；**`meta.policy_events[]`**（门控/策略短路轨迹，无敏感原文）；以及 **可选 provider usage**（`llm_mode` + **`eval_tags` 前缀门禁** + `app.eval.llm-real-*`，见上节「已落地」条）。缺口与后续顺序见 [**`docs/eval/P1_HARNESS_GAP.md`**](eval/P1_HARNESS_GAP.md)。
...
- **主链路**：改写 →（可选）检索 →（可选）工具 → **门控 GUARD** → 上下文增强 → **SSE 流式写作（WRITE）**（与上节顺序一致）；**对话入口**以 **`POST /travel/chat/{conversationId}`**（JSON `query`）为主、GET 查询参数为兼容路径（见 `TravelController` / `docs/ARCHITECTURE.md`）
...
- 要求 target 用结构化字段把执行证据提供给 eval：如 `meta.stage_order[]`、`meta.step_count`、`tool_outcome`/`tool_calls_count`、`hop_trace[]`（多跳）、`low_confidence_reasons[]`、`error_code`、**`config_snapshot_hash`**、**`context_*` / `token_source`**（及可选 **`provider_usage_*`**，受 **`eval_tags`** 与 **`app.eval.llm-real-*`** 门控）等（字段口径以 SSOT 契约为准；本仓已落地子集见 **`docs/eval/P1_HARNESS_GAP.md`**）。
...
- **加到哪里**：本仓为 Postgres 表 **`user_feedback`**（Flyway **V4**）+ **`POST|GET /travel/feedback`**（JWT）；请求体 snake_case。
- **实现**：点赞/点踩（`thumb`）、评分（`rating` 1–5）、可选 **`comment`**（长度见 `app.feedback.max-comment-chars`）；可选 **`eval_case_id` / `eval_tags` / `request_id`** 与 eval 数据集、批跑失败样例、SSE `request_id` 对齐。**后续可选**：运营看板、与 `run.report` 自动 join、反作弊限流等。
...
- **请求体可选字段（与本仓实现对齐）**：**`eval_tags`**（字符串数组，JSON snake_case）：与 **`llm_mode=real`** 联用时用于 **usage 探针抽样**（默认要求至少一条 tag 以 **`cost/`** 等配置前缀开头，见 **`app.eval.llm-real-require-tag-match`** / **`llm-real-required-tag-prefixes`**）；详见 **`README.md`** 与 **`AppEvalProperties`**。
