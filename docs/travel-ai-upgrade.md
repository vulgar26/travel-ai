...
- **P1-0 harness 扩展项**：如显式 **token 预算**（真 token）、断点恢复、Supervisor-Worker 等：**未**按 SSOT 满配。**已收口小步**：`meta.context_truncated` + `context_truncation_reasons[]`、`meta.config_snapshot_hash`（含 `alg/scope`）、以及字符级近似的 `meta.context_token_estimate/context_char_count` 已在评测口落地，用于区分“回归来自预算/配置差异 vs 策略变化”。缺口与后续顺序见 [**`docs/eval/P1_HARNESS_GAP.md`**](eval/P1_HARNESS_GAP.md)。
