...
**更新日期**：2026-04-21（§4：`POST /travel/chat` + `max-query-chars`；`docs/eval.md` 链至 **`LLM_REAL_USAGE_RUNBOOK`**；§5：429 限流体 `error`+`message`；**`meta.config_snapshot_id`**；可选 **`meta.config_snapshot`**；评测 **`meta.policy_events[]`**；**`context_truncation_reasons`** 含检索候选上限与改写截断；**Flyway V3** `eval_conversation_checkpoint`（回放断点 schema + **`EvalCheckpointRepository`** 评测写路径）；含历史 §4/§5 条目）
...
3. **P1-0 harness 分步**：`context_truncated`（含 **`retrieval_candidates_capped`** / **`retrieval_query_line_truncated`** 等）、`config_snapshot_hash`、**`meta.config_snapshot_id`**、可选 **`meta.config_snapshot`**（**`app.eval.config-snapshot-meta-enabled`**）、字符级 **`context_*`/`token_source=estimate`**、可选 **provider usage**（**`llm_mode=real`** + **`eval_tags`** 门禁 + **`app.eval.llm-real-*`**）、**`meta.policy_events[]`**（门控/策略短路轨迹）等已在代码落地；**残余缺口与后续大项**仍以 **`docs/eval/P1_HARNESS_GAP.md`**、**[`docs/eval/LLM_REAL_USAGE_RUNBOOK.md`](eval/LLM_REAL_USAGE_RUNBOOK.md)** 为索引。
